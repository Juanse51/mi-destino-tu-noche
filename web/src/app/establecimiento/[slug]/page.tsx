...
const establecimiento = {
  ...
  cupones: [
    { id: '1', codigo: 'ANDRES20', titulo: '20% de descuento', descripcion: 'Válido en platos principales', tipo_descuento: 'porcentaje', valor_descuento: 20, consumo_minimo: 50000, fecha_fin: '2026-03-31', color_fondo: '#FF6B35', dias_validos: ['lunes','martes','miercoles','jueves','viernes'] },
    { id: '2', codigo: 'ANDRES2X1', titulo: '2x1 en Cócteles', descripcion: 'Aplica en cócteles de la casa', tipo_descuento: '2x1', valor_descuento: null, consumo_minimo: 0, fecha_fin: '2026-02-15', color_fondo: '#8B5CF6', dias_validos: ['jueves','viernes','sabado'] },
  ],
        ...
        <div className="bg-dark-lighter rounded-2xl p-6 md:p-8">
          ...
          <div className="flex border-b border-gray-700 mb-6 overflow-x-auto">
            {(['info', 'cupones', 'menu', 'resenas'] as const).map((tab) => (
              ...
              >
                {tab === 'info' ? 'Información' : tab === 'cupones' ? `🎟️ Cupones (${est.cupones.length})` : tab === 'menu' ? 'Menú' : `Reseñas (${est.total_valoraciones})`}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'cupones' && (
            <div className="space-y-4">
              {est.cupones.length > 0 ? (
                <>
                  <p className="text-gray-400 text-sm">Cupones de descuento disponibles para este establecimiento. Copia el código y preséntalo al momento de pagar.</p>
                  {est.cupones.map((cupon: any) => {
                    const daysLeft = Math.ceil((new Date(cupon.fecha_fin).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
                    return (
                      <div key={cupon.id} className="rounded-xl overflow-hidden border border-gray-700/50" style={{ background: `linear-gradient(135deg, ${cupon.color_fondo}18, ${cupon.color_fondo}05)` }}>
                        <div className="p-5">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div className="flex-1">
                              <div className="text-2xl font-black" style={{ color: cupon.color_fondo }}>
                              ...
                              <h3 className="text-lg font-semibold text-white mt-1">{cupon.titulo}</h3>
                              <p className="text-sm text-gray-400 mt-1">{cupon.descripcion}</p>
                              <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-gray-500">
                                {cupon.consumo_minimo > 0 && <span>Consumo mín: ${cupon.consumo_minimo.toLocaleString()}</span>}
                                <span>•</span>
                                <span>{daysLeft > 0 ? `${daysLeft} días restantes` : 'Expira pronto'}</span>
                                <span>•</span>
                                <span>{cupon.dias_validos.map((d: string) => d.charAt(0).toUpperCase() + d.slice(0, 3)).join(', ')}</span>
                              </div>
                            </div>
                            <div className="flex flex-col items-center gap-2">
                              <div className="bg-dark px-4 py-2.5 rounded-xl border border-gray-700 text-center">
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Código</p>
                                <p className="font-mono font-bold text-lg tracking-wider" style={{ color: cupon.color_fondo }}>{cupon.codigo}</p>
                              </div>
                              <button
                                ...
                                style={{ backgroundColor: copiedCupon === cupon.id ? '#10B981' : cupon.color_fondo }}
                              >
