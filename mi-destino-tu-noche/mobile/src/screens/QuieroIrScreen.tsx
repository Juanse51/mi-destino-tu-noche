...
import { quieroIrApi } from '../api/client';
import { useAuthStore } from '../store/authStore';

export default function QuieroIrScreen() {
  const navigation = useNavigation<any>();
  const { isAuthenticated } = useAuthStore();
  const { data } = useQuery({ queryKey: ['quiero-ir'], queryFn: () => quieroIrApi.getAll(), enabled: isAuthenticated });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyIcon}>📋</Text><Text style={styles.emptyText}>Tu lista está vacía</Text></View>}
      />
    </SafeAreaView>
  );
}
